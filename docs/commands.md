...
| `--mode minimal\|standard` | persist this machine's default agent mode (`defaultMode` in `config.json`); per-run `--mode` and `DSH_CC_MODE` still override. See "Agent mode" above |
...
| `--mode minimal\|standard` | per-run agent mode (default `minimal`; see "Agent mode") |
...
| `--mode minimal\|standard` | agent mode (default `minimal`; see "Agent mode"). A `--session` run resolving a different mode than the live broker errors; a resume keeps the broker's startup mode |
