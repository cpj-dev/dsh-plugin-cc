...
- **minimal** (default): dsh shows better overall capability here on locked two-tool runs. The overlay sets the RL persona (`You are a helpful software engineer assistant.`), sets `includeHarnessIdentity: false` and `includeRuntimeContext: false`, and disables the rest of the dsh-base tool/prompt rows so the model only has bash + `str_replace_editor` for the whole run. The sandboxed filesystem stack, one-shot bash, and context compaction stay composed; bash's `run_in_background` parameter is removed because the job tools that collect a background job are disabled with the rest of the toolset.
