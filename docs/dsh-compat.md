...
| `system-prompt` config accepts `persona`, `includeHarnessIdentity` (default true — prepends `You are an AI agent powered by DeepSeek Harness.`), and `includeRuntimeContext` (default true — sandbox/approval/workspace snapshots as sourced user messages). Both `minimal` and `anchored-standard` overlays set both flags false. Remaining `tool:bash` guidance sections are collapsed by `lib/tool-bootstrap.mjs` (`complete: true` persona section) — `@deepseek-ai/dsh-persona` cannot mount on headless/cc | `packages/core/system-prompt` README | `buildModeOverlayYaml`, `lib/tool-bootstrap.mjs` `applyCompletePersona` |
...
| `@deepseek-ai/dsh-persona` is **scope-only**: mounting it outside an agent preset collides with `deployment:persona` and fails loud. Headless/cc have no preset scope, so complete-persona semantics are applied in `lib/tool-bootstrap.mjs` by replacing assemble `sections` with one `complete: true` RL sentence — not by inserting that package | `packages/preset/persona` README ("No global mount") | `lib/tool-bootstrap.mjs` `applyCompletePersona` |
...
| `system-prompt/assemble` is a waterfall whose result includes `sections` and `tools`. Filtering `assembled.tools` after `next()` changes the model-visible catalog for that request without disabling composition rows. The next assemble after a durable `tool/call` or `assistant/message` on **that session** can return the full list. Same-step parallel tool calls keep the catalog from the assemble that produced them | `packages/core/system-prompt` README; session `request/header` | `lib/tool-bootstrap.mjs` |
| Tool dispatch may still reach a registered tool that was filtered out of the assemble catalog. The bootstrap plugin therefore also listens for `tools/pre-execute` / `tool/pre-execute` and rejects names outside the bootstrap pair while unpromoted. If neither event exists on a given pin, the listen is best-effort (caught) and the assemble filter is the remaining control | tool-bash README (`tools/pre-execute` waterfall) | `lib/tool-bootstrap.mjs` |
