...
| `@deepseek-ai/dsh-sdk-jsonrpc-server` lives at `packages/sdk/server` and is **outside the CLI's dependency closure**, so the launcher's `profiles/node_modules` self-heal never provides it — it must be installed into the cc profile's own node_modules | `packages/sdk/server/package.json`; grep `apps/cli/package.json` deps | `resolveSdkServerDir`, cc profile design |
...
4. Bump the pinned version at the top of this file and in the root README.
