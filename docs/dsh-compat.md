...
| `system-prompt` config accepts `persona`, `includeHarnessIdentity` (default true — prepends `You are an AI agent powered by DeepSeek Harness.`), and `includeRuntimeContext` (default true — sandbox/approval/workspace snapshots as sourced user messages). The plugin's minimal overlay sets both flags false so the documented one-line persona is the actual system text; anchored-standard sets `includeHarnessIdentity: false` only (runtime context returns after promotion via the bootstrap plugin, not this flag) | `packages/core/system-prompt` README | `buildModeOverlayYaml` |
