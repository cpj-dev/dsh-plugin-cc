...
13. Wire snapshot for `--mode anchored-standard` on a **new** session (set `DSH_CC_SNAPSHOT_FILE` and/or read session JSONL `request/header` — do not use `--dump-config` as a wire test). The JSONL recorder writes after `agent/pre-step` (`source: "pre-step"`) and once per step at `step/end` (`source: "request"`, carrying that step's wire header — rc.6 emits `request/header` only when the header *changes*, so a per-event line would skip every steady-state step). Assemble-time empty `contextSourceKinds` is **not** a pass — those injections happen at pre-step. rc.6 stores `model` / `maxTokens` / `reasoningEffort` on `event.data.header.config` (`EpochHeader`), not on the header root; `header.tools` is the wire catalog.
