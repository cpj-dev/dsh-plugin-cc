...
- `spawn.test.mjs` — CVE-2024-27980 contract: rewrite a `.cmd` shim to `node` + JS; plugin-managed two-line POSIX wrapper parse (custom wrappers with extra commands are not rewritten); persisted `{ dshNode, dshBinJs }`; `runCommand` / `binaryAvailable` against a fake `.cmd`; refuse `node.cmd` as the Node executable; a headless run against a fake `.cmd` DSH_BINARY. On windows-latest: live `spawnSync(.cmd)` without shell documents Node's `EINVAL`, and PATH search prefers `npm.cmd` over the extensionless POSIX `npm` shim.
