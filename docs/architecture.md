...
scripts/dsh-bridge.mjs  (subcommand dispatcher; stdout = user-facing result)
        ...
        │                       [--patch <generated agent-mode overlay (minimal default; anchored-standard inserts tool-bootstrap)>]
...
- **Agent mode is a generated `--patch` overlay too, defaulting to minimal.** `minimal` fixes the persona (`includeHarnessIdentity` / `includeRuntimeContext` off) and disables every model-facing dsh-base row except bash and `str_replace_editor`. `standard` is the untouched composition. `anchored-standard` keeps the full registry mounted and inserts `lib/tool-bootstrap.mjs`, which filters the model-visible catalog to the Minimal pair until that session records a durable `tool/call` or `assistant/message`, then returns the assembled catalog. DSH's own preset roster (`dsh-agent-presets`) is unreachable from both plugin paths — the headless bundle and the SDK JSON-RPC server deliberately mount no preset — so the mode lives at the composition layer the plugin does control. `@deepseek-ai/dsh-persona` cannot be inserted here (it is scope-only and collides with the deployment persona). A broker's mode is fixed at spawn (same lifecycle as its permission mode).
