...
- **已有已构建的源码目录：**运行 `/dsh:setup --harness <absolute-path>`。目录必须已经执行过 `pnpm install && pnpm run build:lib`；插件不会代为编译。
- **已有 `DSH_BINARY`，但缺少 `cc` profile：**仍需运行 `/dsh:setup`。它会把 `@deepseek-ai/dsh-sdk-jsonrpc-server@<pin>` 及其已发布的 peers 装进 profile。
