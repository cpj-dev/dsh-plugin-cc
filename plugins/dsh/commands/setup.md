...
1. **Get dsh.** Default: install `@deepseek-ai/dsh@<HARNESS_NPM_VERSION>` into the plugin data directory (`npm install --prefix`), persist `{ dshNode, dshBinJs }` so every spawn is `node` + `lib/bin.js` (a POSIX `bin/dsh` wrapper is extra on Unix; Windows does not write a `.cmd`). A persisted source install (old `harnessCheckout` / `dshInstall: harness`) is migrated to that pin. `--harness <checkout-path>` uses an already-built DeepSeek Harness checkout instead (the plugin does not run `pnpm install` / `build:lib`; `packages/sdk/server` must exist). A later no-args setup migrates back to npm. Requirements: Node >= 22.19 (harness floor; the plugin itself needs only >= 20), `npm` (default path), and `pnpm` for profile plugin add (`corepack enable` when missing). Do not follow npm `latest`/`next`.
