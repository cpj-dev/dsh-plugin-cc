...
- `--model <name>` / `--effort low|high|max`: per-run model selection (ignored on `--resume`; the broker keeps its startup model). Defaults: `deepseek-v4-pro` at effort `max`. `low` is in the official schema; `medium` is not.
